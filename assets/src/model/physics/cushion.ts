...
export class Cushion {
   ...
   */
  static bounceAny(ball: Ball, t: number, hasPockets: boolean = true, cushionModel = bounceHanBlend): number | undefined {
    const futurePosition = ball.futurePosition(t)

    if (Cushion.willBounceLong(futurePosition, hasPockets)) {
      const dir = futurePosition.y > TableGeometry.tableY ? -Math.PI / 2 : Math.PI / 2
  ...
  static willBounceShort(futurePosition, hasPockets) {
    if (!hasPockets) {
      return Cushion.willBounceShortSegment(TableGeometry.Y, -TableGeometry.Y, futurePosition)
    }
  ...
  static willBounceLong(futurePosition, hasPockets) {
    if (!hasPockets) {
      return Cushion.willBounceLongSegment(-TableGeometry.X, TableGeometry.X, futurePosition)
    }
    return (
  ...
  private static willBounceShortSegment(top: number, bottom: number, futurePosition: Vec3): boolean {
    return (
