...
export function surfaceVelocityFull(v, w) {
  return new Vec3(v).addScaledVector(upCross(w), R)
}
...
export function sliding(v, w) {
  ...
  delta.v = new Vec3(norm(va).multiplyScalar(-muS * g))
  delta.w = new Vec3(norm(upCross(va)).multiplyScalar(((5 / 2) * muS * g) / R))
...
export function forceRoll(v, w) {
  const wz = w.z
  w = new Vec3(upCross(v).multiplyScalar(1 / R))
  w.setZ(wz)
}
