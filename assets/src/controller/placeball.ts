...
export class PlaceBall extends ControllerBase {
  ...
  override onFirst() {
    ...
    if (this.container.rules.allowsPlaceBall()) {
      cueball.pos = new Vec3(this.container.rules.placeBall())
    }
  ...
  moveTo(dx, dy) {
    const delta = new Vec3(dx, dy)
    let ballPos = this.container.table.cueball.pos.add(delta)
    ballPos = new Vec3(this.container.rules.placeBall(ballPos))
