...
export class Cue {
  ...
  hit(ball: Ball) {
    ...
    ball.vel = new Vec3(unitAtAngle(aim.angle).multiplyScalar(aim.power))
    ball.rvel = new Vec3(cueToSpin(aim.offset, ball.vel))
  }
  ...
  setSpin(offset: Vector3, table: Table) {
    ...
    this.aim.offset = new Vec3(offset)
    this.avoidCueTouchingOtherBall(table)
    this.updateAimInput()
  }
  ...
  moveTo(pos) {
    ...
    this.mesh.position = new Vec3(pos.clone().add(offset).add(distanceToBall))
    this.helperMesh.position = new Vec3(pos)
    this.placerMesh.position = new Vec3(pos)
