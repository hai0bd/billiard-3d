import { Pze, Pzs, c0, s0, bounceHan, cueToSpin, bounceHanBlend, } from "./model/physics/physics"
import { Vec3 } from "cc"
...
function reportConstants() {
  ...
  const v = new Vec3(new Cue().maxPower, 0, 0)
  const w = cueToSpin(new Vec3(0.5), v)
...
function lineGraph1() {
  ...
  for (let i = -180; i <= 180; i += 30) {
    x.push(i)
    const v = new Vec3(0.2 * R, 0.0, 0)
    const w = new Vec3(0.0, 0.0, i * R)
    y1.push(Pze(c0(v)))
    y2.push(Pzs(s0(v, w)))
  }
...
function lineGraph2() {
  ...
  for (let i = -180; i <= 180; i += 30) {
    x.push(i)
    const v = new Vec3(150 * R, 0, 0)
    const w = new Vec3(0.0, i * R, 0)
    y1.push(Pze(c0(v)))
    y2.push(Pzs(s0(v, w)))
  }
...
function lineGraph3() {
  ...
  for (let i = -80; i <= 80; i += 10) {
    ...
    const v = new Vec3(Math.cos(rad) * R, Math.sin(rad) * R, 0)
    v.multiplyScalar(s)
    const w = new Vec3(0.0, 0, -10 * R)
    y1.push(Pze(c0(v)))
    y2.push(Pzs(s0(v, w)))
  }
